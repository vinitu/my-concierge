...
```text
gateway -> assistant-api -> redis -> assistant-worker -> redis -> assistant-api -> gateway
```
...
5. `assistant-worker` reads the execution job from Redis.
6. `assistant-worker` runs assistant logic.
7. `assistant-worker` persists conversation state and calls `assistant-memory` when durable memory is needed.
8. `assistant-worker` publishes `thinking`, `completed`, or `failed` run events to Redis.
...
gateway-web -> assistant-api -> redis -> assistant-worker
assistant-worker -> redis -> assistant-api
...
8. `assistant-worker` may publish `thinking` run events while the LangChain.js run is active.
